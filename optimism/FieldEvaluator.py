...
if __name__ == "__main__":
    ...
    spaces = [u_space, time_space]
    DISPLACEMENT = 0
    TIME = 1

    inputs = (Value(DISPLACEMENT), Value(TIME), Gradient(DISPLACEMENT))

    def scaled_disp_grad(u, t, dudX):
        return t*dudX

    fs = FieldEvaluator([u_space, time_space], inputs, mesh, quad_rule)
    ...
    disp_grads = fs.evaluate(scaled_disp_grad, mesh.coords, U, time)
    print(f"{disp_grads=}")
