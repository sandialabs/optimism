...
def evaluate_on_block(functionSpace, U, stateVars, dt, func, block,
    ...
    """
    fs = functionSpace
    compute_elem_values = jax.vmap(evaluate_on_element, (None, None, 0, None, 0, 0, 0, 0, None, None, *tuple(None for p in params)))
...
def integrate_element_from_local_field(elemNodalField, elemNodalCoords, elemStates, dt, elemShapes, elemShapeGrads, elemVols, func, *params, modify_element_gradient=default_modify_element_gradient):
...
def evaluate_on_element(U, coords, elemStates, dt, elemShapes, elemShapeGrads, elemVols, elemConn, kernelFunc, modify_element_gradient, *params):
    ...
    elemGrads = compute_element_field_gradient(U, coords, elemShapes, elemShapeGrads, elemVols, elemConn, modify_element_gradient)
    elemXs = interpolate_to_element_points(coords, elemShapes, elemConn)
    elemParams = tuple(interpolate_to_element_points(p, elemShapes, elemConn) for p in params)
    vmapArgs = 0, 0, 0, 0, None, *tuple(0 for p in params)
    fVals = jax.vmap(kernelFunc, vmapArgs)(elemVals, elemGrads, elemStates, elemXs, dt, *elemParams)
    return fVals
