...
def project_props(fspace, props_in):
    props = FunctionSpace.interpolate_to_points(fspace, props_in)
    props = props.reshape((props.shape[0], props.shape[2]))
    return props
...
class ElementPropertyOptimization:
    ...
    def reload_mesh(self):
        ...
        # Ryan change stuff below
        # Right now it's making a nodal field that's a sin wave based on the y coordinates
        # you need to reshape it to be N-nodes x 1 or N-nodes x N-props if there's more
        # than one prop to optimize
        self.props = 150. * np.sin(16. * self.mesh.coords[:, 1]) + 1.
        self.props = self.props.reshape((self.props.shape[0], 1))
    ...
    def run_simulation(self):
        ...
        # project
        quad_rule_project = QuadratureRule.create_quadrature_rule_on_triangle(degree=1)
        func_space_project = FunctionSpace.construct_function_space(self.mesh, quad_rule_project)

        def energy_function_all_dofs(U, p):
            internal_variables = p.state_data
            props = project_props(func_space_project, p.prop_data)
        ...
        def save_exodus_outputs(Uu, p, step):
            ...
            writer.add_nodal_field(name='properties', nodalData=self.props, fieldType=VTKWriter.VTKFieldType.SCALARS)
            writer.add_cell_field(name='el_props', cellData=props, fieldType=VTKWriter.VTKFieldType.SCALARS)
            writer.write()
        ...
        for step in range(1, steps_per_stage+1):
            ...
            U = self.create_field(Uu, p.bc_data)
            ivs = mech_funcs.compute_updated_internal_variables(U, p.state_data, project_props(func_space_project, p.prop_data), self.dt)
    ...
    def setup_energy_functions(self):
        ...
        def energy_function_all_dofs(U, ivs, coords):
            ...
            # TODO add props as a input to compute_strain_energy method
            return mech_funcs.compute_strain_energy(U, ivs, project_props(func_space_project, self.props), self.dt)
