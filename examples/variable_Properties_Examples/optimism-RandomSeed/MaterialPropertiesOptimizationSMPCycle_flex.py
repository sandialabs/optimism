...
class MaterialPropertiesOptimization:
    ...
    def __init__(self):
        self.targetSteps = [10, 20, 30, 40, 50, 60, 70, 80] # step that the target loads and displacments should occur on 
        ...
        self.targetDisplacement = [0.025, 0.05, 0.08, 0.15, 0.16, 0.15, 0.13, 0.11] # target displacement 
        self.targetForces = [-0.5, -1, -1.5, -2, -1.5, -1, -0.5, 0] # target plateau response
        self.scaleObjective = 1.0 # -1.0 to maximize, 1.0 to minimize
        ...
        # read in the mesh, in this example project, a density value will be assigned to the elements, so a seeded mesh is not necessary
        ...
        surfaceYCoords = self.mesh.coords[self.mesh.nodeSets['xplus_nodeset']][:,1]
        ...
        self.targetXCoords = -B*(1 - np.sin(np.pi * (1.5 + surfaceYCoords) / L))
        # calculate the target coordinates from the initial beam and target bend
        self.targetXCoords += self.mesh.coords[self.mesh.nodeSets['xplus_nodeset']][:,0]
    ...
    def run_simulation(self):
        ...
        def write_vtk_output(Uu, p, step):
            U = self.create_field(Uu, p.bc_data)
            plotName = './output/output-'+str(step).zfill(3)
    ...
    def compute_L2_norm_difference(self, uSteps, pSteps, properties, nodal_forces):
        numerator = 0.0
        denominator= 0.0
        finalXCoords = self.mesh.coords[self.mesh.nodeSets['xplus_nodeset']][:,0]
        for i in range(0, len(self.targetXCoords)):
            ...
            diff = finalXCoords[i] - self.targetXCoords[i]
            numerator += diff*diff
            denominator += self.targetXCoords[i]*self.targetXCoords[i]
