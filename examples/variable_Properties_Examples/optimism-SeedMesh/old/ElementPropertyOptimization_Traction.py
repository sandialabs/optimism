...
class ElementPropertyOptimization:
    def __init__(self):
        ...
        if self.writeOutput:
          self.output_file = 'EXO_files/output_ellipse_test_Seeded_traction.exo'
        ...
        self.mat_model = Neohookean_VariableProps.create_material_model_functions(Props)
    ...
    def run_simulation(self):
        ...
        def energy_function_all_dofs(U, p):
            ...
            strainEnergy = mech_funcs.compute_strain_energy(U, internal_variables, props, self.dt)
            loadPotential = Mechanics.compute_traction_potential_energy(func_space, U, self.lineQuadRule, self.mesh.sideSets['yplus_sideset'],
                                                                        lambda x, n: np.array([0.0, -F]))
