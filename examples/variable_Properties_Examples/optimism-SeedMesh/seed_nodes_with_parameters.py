...
def node_centroid_distance(x, y, z):
    nodeCent = [0]*len(x)
    for i in range(len(x)):
        x_coord = x[i]
        y_coord = y[i]
        z_coord = z[i]
        #param[i] =  mult * np.sqrt(x_coord**2 + y_coord**2 + z_coord**2)/10 
        nodeCent[i] =  np.sqrt(x_coord**2 + y_coord**2 + z_coord**2)
    return nodeCent
...
def add_properties_to_mesh(mesh_file, block_id, property_names, property_funcs, constant_params, property_params, multiplier):
    ...
    node_centroid_norms = node_centroid_distance(x_coords, y_coords, z_coords)

    maxNorm = max(node_centroid_norms)
    minNorm = min(node_centroid_norms)


    # assign the property values to each property in the elements
    # for i in range(len(property_names)):
    #     if constant_params[i] < 1:
    #         new_exo.put_element_variable_values(block_id, property_names[i], 1, property_funcs[0](element_x, element_y, element_z,minNorm, maxNorm, property_params[i], multiplier[i]))
    ...
    #     else:
    #         new_exo.put_element_variable_values(block_id, property_names[i], 1, property_funcs[1](element_x, element_y, element_z,  minNorm, maxNorm, property_params[i], multiplier[i]))
     
    # print(new_exo.get_node_variable_number())
    # num_nvars = new_exo.get_node_variable_number()

    # set the number of variables in the nodes to the number of variables we want to assign
    new_exo.set_node_variable_number(len(property_names))
    # print(range(len(property_names)))

    # iterate throught the properties, assigning the variable names to the nodes
    for i in range(len(property_names)):
        # print(i)
        new_exo.put_node_variable_name(property_names[i], i+1)
    # print(new_exo.get_node_variable_names())
    # print(new_exo.get_node_variable_number())

    for i in range(len(property_names)):
        if constant_params[i] < 1:
            # print(i)
            # put_variable_values('EX_ELEM_BLOCK', blockId, name, step, values)
            new_exo.put_node_variable_values(property_names[i], 1, property_funcs[0](x_coords, y_coords, z_coords,minNorm, maxNorm, property_params[i], multiplier[i]))
            #print((property_funcs[0](x_coords, y_coords, z_coords,minNorm, maxNorm, property_params[i], multiplier[i])))
            #print(min(property_funcs[0](x_coords, y_coords, z_coords,minNorm, maxNorm, property_params[i], multiplier[i])))
            #print(max(property_funcs[0](x_coords, y_coords, z_coords,minNorm, maxNorm, property_params[i], multiplier[i])))
        else:
            new_exo.put_node_variable_values(property_names[i], 1, property_funcs[1](x_coords, y_coords, z_coords, minNorm, maxNorm, property_params[i], multiplier[i]))
...
#add_properties_to_mesh('ellipse_test.exo', 1, ['Youngs_Modulus', 'nu'], [param_function_linear_y_decreasing, param_function_const], [0, 1], [1010, 0.48], [990, 0])

# functional
add_properties_to_mesh('./EXO files/solidTest_2.exo', 1, ['light_dose', 'nu'], [param_function_linear_y_increasing, param_function_const], [0, 1], [260, 0.48], [240, 0])
