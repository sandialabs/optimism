...
class MaterialPropertiesOptimization:
    ...
    def __init__(self):
        ...
        self.targetForces = [-1.6, -3.1, -3.5, -3.6] # target plateau response
    ...
    def get_gradient(self):
        ...
        uSteps = np.stack([self.state[i][0] for i in range(0, self.steps+1)], axis=0)
        pSteps = [self.state[i][1] for i in range(0, self.steps+1)]
        df_du, gradient = grad(self.compute_total_work_minus_free_energy, (0, 2))(uSteps, pSteps, parameters, energyFuncs.nodal_forces, jit(energyFuncs.energy_function_coords))
...
if __name__ == '__main__':
    ...
    materialProperties = np.zeros(mpo.mesh.conns.shape[0]).tolist()
