...
from optimism import PostProcessor
...
if __name__ == '__main__':
    mesh = ReadExodusMesh.read_exodus_mesh('./pad_unit_cell.g')
    ...
    def constraint_function(Uu, p):
        ...
        contact_dist_1 = closest_distance_func(mesh, U, quad_rule_face, 
                                               interaction_list[0], contact_edges_left).ravel()
        contact_dist_2 = closest_distance_func(mesh, U, quad_rule_face, 
                                               interaction_list[1], contact_edges_right).ravel()
        contact_dist_3 = closest_distance_func(mesh, U, quad_rule_face, 
                                               interaction_list[2], contact_edges_left).ravel()
        contact_dist_4 = closest_distance_func(mesh, U, quad_rule_face, 
                                               interaction_list[3], contact_edges_right).ravel()
    ...
    def run_with_contact():
        ...
        vtk_writer = PostProcessor.setup_vtk_post_processor('output-' + str(step).zfill(3), mesh)
        PostProcessor.write_nodal_field_to_vtk(vtk_writer, 'displ', Uu, p, create_field)
        vtk_writer.write()
